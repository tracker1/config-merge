...
const readYaml = async (name, filepath) => {
  try {
    ...
    console.error(
      `Error reading yaml file: ${filepath}`,
...
const getImageList = async directory => {
  try {
    return (await fs.readdir(directory))
      ...
      .reduce(
        (o, f) =>
          Object.assign(o, { [path.basename(f)]: path.join(directory, f) }),
        {}
      );
  } catch (error) {
...
const getConfigFiles = async directory => {
  try {
    ...
    return (await Promise.all(
      files.map(f => readYaml(f, path.join(directory, f)))
...
async function loadTargets(inputDirectory) {
  ...
  const targets = await Promise.all(
    results
      .filter(r => !/^[_\.]/.test(r))
      .map(r => loadConfig(path.resolve(inputDirectory, r)))
  );
